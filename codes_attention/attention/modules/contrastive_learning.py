...
class EEGContrastiveLearning(LightningModule):
    ...
    def forward(self, eeg, m_v, m_d, m_b, m_o):
        y_eeg, z_eeg = self.encoder_raw_e(eeg)
        y_v, z_v = self.encoder_vocal(m_v)
        y_d, z_d = self.encoder_drum(m_d)
        y_b, z_b = self.encoder_bass(m_b)
        y_o, z_o = self.encoder_others(m_o)

        return y_eeg, y_v, y_d, y_b, y_o, z_eeg, z_v, z_d, z_b, z_o
