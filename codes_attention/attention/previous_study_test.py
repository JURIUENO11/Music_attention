    ...
    for s in range(8):   
        def generator(dataset):
            for i in range(len(dataset)):
                e, a0, a1, a2, a3, label, attention_score, subject, song_id = dataset[i]
                # if attention_score < 4:
                #     continue
                # if attention_score < 4 or subject != s:
                #     continue
                if subject != s:
                    continue
                a0 = a0.mean(axis=0) 
                ...
                if e_csp.shape[0] != 768:
                    pad_width = 768 - e_csp.shape[0]
                    e_csp = np.pad(e_csp, ((0, pad_width), (0,0)), mode='constant')

                if fused0.shape[0] != 768:
                    pad_width = 768 - fused0.shape[0]
                    fused0 = np.pad(fused0, ((0, pad_width), (0,0), (0,0)), mode='constant')

                if fused1.shape[0] != 768:
                    pad_width = 768 - fused1.shape[0]
                    fused1 = np.pad(fused1, ((0, pad_width), (0,0), (0,0)), mode='constant')

                if fused2.shape[0] != 768:
                    pad_width = 768 - fused2.shape[0]
                    fused2 = np.pad(fused2, ((0, pad_width), (0,0), (0,0)), mode='constant')

                if fused3.shape[0] != 768:
                    pad_width = 768 - fused3.shape[0]
                    fused3 = np.pad(fused3, ((0, pad_width), (0,0), (0,0)), mode='constant')
        ...
        test_dataset.set_eeg_normalization(args.eeg_normalization, args.clamp_value)
        test_dataset.set_other_parameters(args.eeg_length, args.audio_clip_length, args.shifting_time, args.start_position)
        random.seed(42)
        test_random_numbers = [random.randint(
            0, args.window_size - args.eeg_length - 1) for _ in range(1200)]
        test_dataset.set_random_numbers(test_random_numbers)
        e, a0, *_ = test_dataset[0]
        print('e',e.shape)
        print('a0',a0.shape)
        print('a0',a0.mean(axis=0).shape)
        print(torch.equal(a0[0], a0[1]))
        ...
        # --------------------------------------------------------
        example_batch = next(iter(test_tf_dataset))
        ...
        model.load_weights("previous_paper_model.ckpt")

        print("[[[ WEIGHTS LOADED SUCCESSFULLY ]]]")

        results = model.evaluate(test_tf_dataset, verbose=2)
