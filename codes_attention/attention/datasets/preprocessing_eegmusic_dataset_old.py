...
class Preprocessing_EEGMusic_dataset(Dataset):
    ...
    def __init__(
        ...
        self.test_data_length = 768
        self.whole_audio_length = 15
        self.audio_length = 0
        self.audio_sample_rate = 44100
        self.train_test_splitting = "random_split_3s"
        self.random_numbers = []
        ...
        self.df = self._get_file_list(
            self._base_dir, self.subset).reset_index(drop=True)
    ...
    def getitem(self, n, isClip=True):
        ...
        song = (self._get_song_name(self.df_subset.iloc[n, 1]))
        song = np.array(1)

 
        if isClip:
            eeg_length = eeg.size(1)
            eeg_start = random.randint(int((self.audio_clip-3)/2*self.eeg_sample_rate), int(
                eeg_length-self.eeg_clip_length-(self.audio_clip-3)/2*self.eeg_sample_rate-1))
            ...
            for k, audio in enumerate(audio_list):
                audio_start = int(eeg_start/self.eeg_sample_rate * self.audio_sample_rate)+int(
                    (self.audio_clip-3)/2*self.eeg_sample_rate)
        ...
        if eeg.shape[1] != 3**7:
            ...
            eeg = F.pad(eeg, padding)
        padding2 = (int((3**10-self.audio_length/3)/2),
                    int((3**10-self.audio_length/3)/2))

        for k, audio in enumerate(audio_list):
            audio_list[k]["value"] = F.pad(audio["value"], padding2)
            if audio["value"].shape[1] != 3**10:
                padding2 = (0, 3**10-audio["value"].shape[1])
                audio_list[k]["value"] = F.pad(audio["value"], padding2)
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, Tensor]:
        eeg, audio0, audio1, audio2, audio3, task, attention_score, subject, song = self.getitem(n)
        return eeg, audio0, audio1, audio2, audio3, task, attention_score, subject, song
    ...
    def _get_file_list(self, root, subset):
        ...
        Audio_path_list = [{'task': int(f.parts[9]), 'name': self._get_song_name(
        ...
        for idx, r_path in enumerate(EEG_path_list):
            ...
            r_subject = r_part[9]
            r_song = self._get_song_name(r_part[11])
            r_task = int(r_part[12])
            attention_score = int(r_part[13])

            c_audio_list = list(filter(lambda x: self._get_song_name(
                x["name"]) == self._get_song_name(r_song), Audio_path_list))
...
class Preprocessing_EEGMusic_Test_dataset(Preprocessing_EEGMusic_dataset):
    ...
    def getitem(self, n, isClip=True):
        ...
        return eeg, padded_audio0, padded_audio1, padded_audio2, padded_audio3, task, attention_score, subject, song
